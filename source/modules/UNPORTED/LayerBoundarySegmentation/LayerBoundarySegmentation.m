function [Resources ErrorMsg] = LayerBoundarySegmentation(client_server, image_url, user, password)
...
% 
    ...
    %load_training_data();
    retina.train_gt = load_GObj_data(retina.train_image_url,'training_GT_data',num_segm_layers);
...
function get_image_info()
    ...
    if(isempty(temp) || ~strcmp(temp,'layer_boundary_segmentation'))
        userErrorMsg = 'Selected Image has no valid tag information. Required tag - dataset_label:layer_boundary_segmentation. Unable to perform Segmentation';
        error(userErrorMsg);
    end
          ...
          % retina.num_layers = size(polyline);
           retina.num_layers = num_layers;
            ...
            for layer = 0:size(polyline)-1
               ...
               loc = 1;
               while(strcmp(polyline.get(layer).name, retina.segm_layers{loc}) ~= 1)    
                   loc = loc + 1;
                   if(loc > num_layers)
                        break;
                   end            
...
function retina_save_segm_out()
    ...
    %add GObjects to it
    retina.GObject = BQ.createGObject('polyline_set','AC_Output_SEG_Data');
    ...
    BQ.deleteTag(retina.BQ_image_url,'AC_Seg_GObj');
    tag = BQ.addTag(retina.BQ_image_url,'AC_Seg_GObj',char(retina.GObjectURL));
    ...
    BQ.deleteTag(retina.BQ_image_url,'AC_Train_Image');
    tag2 = BQ.addTag(retina.BQ_image_url,'AC_Train_Image',char(retina.train_image_url));
...
function retina_save_segm_mask()
    ...
    for layer=1:retina.num_layers
        ...
        [line_ind,xy_line,mask] = f_pt2mask2(fliplr(retina.segm_out{layer}),[r c],1);
    ...
    BQ.deleteTag(retina.BQ_image_url,'AC_Seg_Mask');
    maskTag = BQ.addTag(retina.BQ_image_url, 'AC_Seg_Mask', char(retina.segm_mask_url));
    ...
    BQ.deleteTag(BQ_segm_image_url,'AC_Seg_GObj');
    tag1 = BQ.addTag(BQ_segm_image_url,'image', retina.image_url);
    if(isempty(tag1)); error(char(BQError.getLastError())); end    
    tag2 = BQ.addTag(BQ_segm_image_url,'AC_Seg_GObj',char(retina.GObjectURL));
    if(isempty(tag2)); error(char(BQError.getLastError())); end   
