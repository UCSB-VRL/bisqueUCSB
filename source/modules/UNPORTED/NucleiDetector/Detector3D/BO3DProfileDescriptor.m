function nucleiFilteredPositions = BO3DProfileDescriptorNEW(imageIn, nucleiPositions, descriptorTable, cellSize, resolutionXY, resolutionZ, scaleXY, scaleZ )
...
cellsizeXY = cellSize; % Cell range between 2.8-5.0 microns
cellsizeZ = cellSize;
...
%for j=1:tableSize
    ...
    if (m<distancemaxXY) && (abs(nucleiPositions_tmp(ix,3)- nucleiPositions_tmp(jx,3))<=distancemaxZ)
        ...
        lineMask = bresenham_line_3D(nucleiPositions_tmp(ix,1), nucleiPositions_tmp(ix,2), nucleiPositions_tmp(ix,3),...
                                     nucleiPositions_tmp(jx,1), nucleiPositions_tmp(jx,2), nucleiPositions_tmp(jx,3), lineMask);
        profileLine = double(imageIn(logical(lineMask)));
        profileLength = sum(sqrt(double(1.0 + diff(profileLine).^2)));
        
%         subplot(1,2,1), imagesc(imageIn(:,:,nucleiPositions_tmp(ix,3))), colormap gray;
%         hold on 
...
% 
%         subplot(1,2,2), imagesc(imageIn2(:,:,nucleiPositions_tmp(ix,3))), colormap gray;
%         hold on 
%         hold on 
%         plot(nucleiPositions_tmp(ix,2), nucleiPositions_tmp(ix,1), 'yo')
%         hold on          
