...
class PythonScriptWrapper(object):
    ...
    def getImages(self,bq):
       ...
       self.data_dir = os.path.join(self.options.stagingPath, data_dir)
    ...
    def uploadImgService(self, bq, files):
        ...
        resource = ET.Element('image', name='ModuleExecutions/EnhancedReconstruction/'+filename)
    ...
    def run(self):
        ...
        log.info('Running enhancement script...')
        self.out_dir = os.path.join(self.options.stagingPath, out_dir)
		
        try:
            bq.update_mex('Running prediction script...')
            self.outfiles = predict.main(self.data_dir, self.out_dir)
            log.info('Enhancement successful.')
        except (Exception, ScriptError) as e:
            log.exception("Exception during segmentation")
            bq.fail_mex(msg = "Exception during prediction: %s" % str(e))
            return

        
        log.info('Uploading enhanced images...')
        try:
            bq.update_mex('Uploading enhanced images...')
