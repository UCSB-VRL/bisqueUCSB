...
%crop to allow window_size to fit inside
hr=mod(h,window_size);
wr=mod(w,window_size);
im=im(floor(hr/2+1):floor(hr/2+1)+floor(floor(h-hr/2)/window_size)*window_size-1,floor(wr/2+1):floor(wr/2+1)+floor(floor(w-wr/2)/window_size)*window_size-1);
