...
class itkFastMarchingSegmentationPY:
    """ITK Fast Marching Segmentation Module
    """
    def __init__(self):
        pass
    ...
    def loadImage(self, url):
        ...
        log.debug ('itkFastMarchingSegmentationPY - response:  ' +  str(content)  + '||||||||||' +url)
    ...
    def processImage(self):        
        ...
        if int(self.image.z) == 1  and  int(self.image.t) == 1 and (int(self.image.ch) ==1 or int(self.image.ch) == 3):
			...
			seg.doSegment(tempinfile,tempoutfile, self.seedX, self.seedY, self.Sigma, self.SigmoidAlpha, self.SigmoidBeta, TimeThreshold, StoppingValue)
			...
			print 'output resp =',str(resp)
			print 'output content =',str(content)
    ...
    def main(self, client_server, image_url,seedX, seedY, Sigma, SigmoidAlpha, SigmoidBeta,user=None, password=None):
		...
		self.seedX = int(seedX)
		self.seedY = int(seedY) 
		self.Sigma = float(Sigma)
		self.SigmoidAlpha = float(SigmoidAlpha)
		self.SigmoidBeta = float(SigmoidBeta)

		self.processImage()
		self.tag = BQTag
		if self.seedX in range(78,83):
			self.tag.name = "Left_Ventricle_Mask"
		elif self.seedX in range(97,101):
			self.tag.name = "Right_Ventricle_Mask"
		elif self.seedX in range(54,58):
			self.tag.name = "White_Matter_Mask"
		elif self.seedX in range(38,42):
			self.tag.name = "Grey_Matter_Mask"
		else:
			self.tag.name = "Unknown_Region_Mask"

		self.tag.value = self.image_out_url 
		resp = self.postTag(self.image, self.tag)
    	
#if __name__ == "__main__":
#	m = itkFastMarchingSegmentationPY()
#        m.main()
