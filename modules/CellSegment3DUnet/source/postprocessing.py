...
def label_segmentation(tiff_filename):
    ...
    for idx, arr in enumerate(img[:n_max]):
        ...
        if random_cmap: im_arr = (255*darken*cmap.map(arr)).astype(np.uint8)
        else: im_arr = (255*darken*cm.jet(arr/arr.max())[:,:,:3]).astype(np.uint8)
...
#          else: print(f'Bad index: {index}')  #the mean is outside of cell
    ...
    tifffile.imsave(tiff_filename, labeled_image, compress=1, metadata={ 'colormap': 'rgb'}, extratags=[(270,'s',1,imMeta)]) 
...
if __name__== "__main__":
    main(bq, 'source/prob_map/','source/result/', '/source/data/celldata', min_distance, label_threshold, black_threshold)
