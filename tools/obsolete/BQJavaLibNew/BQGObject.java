...
public class BQGObject extends BQObject{
    ...
    public void setParent(BQResource parent){
        ((BQObject)parent).addGObject(this);
        this.parent = (BQObject)parent;
    }
    ...
    public void addVertices( double[] x, double[] y, double[] z, double[] t, int[] ch, int[] index){
        ...
        for (int i=0; i < x.length; i++)
            vertices.add(new BQVertex(  (Double)x[i], (Double)y[i], (Double)z[i],
                                        (Double)t[i], (Integer)ch[i], (Integer)index[i]));
    }
